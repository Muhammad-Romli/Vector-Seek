...
def convert_file_dict_to_dict_metadata(list_of_file_dict: list[dict[str, str]]) -> list[list[dict[str, str | list[str] | None]]]:
    # each dict that gonna be returned is list of dict that that contain from_file, title, and splitted content
    list_of_dict_metadata = []
    for file_dict in list_of_file_dict:
        file_name = file_dict["from_file"]
        file_content = file_dict["content"]
        dict_metadata = markdown_to_dict_metadata(file_name, file_content)
        dict_metadata: list[dict[str, str | list[str] | None]]
